...
describe("TestCasesService", () => {
	...
	describe("getTestCasesByProblemId", () => {
		it("delegates to repository.getTestCasesByProblemId", async () => {
			mockRepo.getTestCasesByProblemId.mockResolvedValue([
				mockTestCase,
			] as any);

			const result =
				await service.getTestCasesByProblemId("problem-id-1");
	...
	describe("addTestCaseToProblem", () => {
		it("delegates to repository.addTestCaseToProblem", async () => {
			mockRepo.addTestCaseToProblem.mockResolvedValue(
				mockTestCase as any,
			);
			const data = { input: "1", expectedOutput: "2", isSample: false };

			const result = await service.addTestCaseToProblem(
				"problem-id-1",
				data,
			);
